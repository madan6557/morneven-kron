...
class TeamDriveScopeProbe internal constructor(
    ...
    suspend fun verifyMemberAccess(): TeamScopeProbeResult = withToken { account, token ->
        ...
        TeamScopeProbeResult.Ready(
            if (workspace.capabilities.canWrite) "Scope drive.file lintas akun berhasil untuk Editor."
            else "Scope drive.file lintas akun berhasil untuk Viewer.",
        )
    }
    ...
    fun create(activity: Activity): TeamDriveScopeProbe {
        ...
        return TeamDriveScopeProbe(
            ...
            authorization = AuthorizationClientDriveSession(
                accountSelector = AndroidCredentialManagerAccountSelector(activity, BuildConfig.GOOGLE_WEB_CLIENT_ID),
